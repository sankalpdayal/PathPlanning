      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			//if previous list almost empty, which is when the car is starting
			if (prev_size < 2){
				//ise two points that make the path tangent to the car
				...
				//redefine refernce state as previous path and end points
			...
			//reseting points to reference values
			...
			//fill all the previous points
			for (int i = 0; i<previous_path_x.size();i++){
				next_x_vals.push_back(previous_path_x[i]);
				next_y_vals.push_back(previous_path_y[i]);
			}
			
			//calculate spline at desired points
			double target_x = 30.0;
			double target_y = s(target_x);//evaluating spline
